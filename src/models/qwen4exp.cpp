...
void llama_model_qwen4exp::load_arch_hparams(llama_model_loader & ml) {
    // absent in a normal file, 1 in an MTP draft file. n_layer() is n_layer_all - n_layer_nextn.
...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    // MTP draft block at index n_layer. TENSOR_SKIP unless the caller asked for the draft.
    ...
    for (int il = (int) n_layer; il < (int) (n_layer + hparams.n_layer_nextn); ++il) {
        ...
        layer.hc_attn_norm   = create_tensor(tn(LLM_TENSOR_HC_ATTN_NORM,   "weight", il), { hc_dim }, mtp_flags);
        ...
        layer.hc_ffn_norm    = create_tensor(tn(LLM_TENSOR_HC_FFN_NORM,    "weight", il), { hc_dim }, mtp_flags);
        ...
        const int64_t idx_dim = hparams.indexer_head_size;
        layer.index_q_proj = create_tensor(tn(LLM_TENSOR_INDEXER_Q_PROJ, "weight", il), { n_embd, hparams.indexer_n_head * idx_dim }, mtp_flags);
        layer.index_k_proj = create_tensor(tn(LLM_TENSOR_INDEXER_K_PROJ, "weight", il), { n_embd, idx_dim }, mtp_flags);
        layer.index_q_norm = create_tensor(tn(LLM_TENSOR_INDEXER_Q_NORM, "weight", il), { idx_dim }, mtp_flags);
        layer.index_k_norm = create_tensor(tn(LLM_TENSOR_INDEXER_K_NORM, "weight", il), { idx_dim }, mtp_flags);
...
ggml_tensor * llama_model_qwen4exp::graph::build_conv_state_at(
    ...
    const size_t off_b    = row_offset * ggml_element_size(conv_states_all);
    ...
    auto save_slot = [&](int64_t s_idx, int64_t s_slot) {
        ...
        ggml_tensor * dst = ggml_view_2d(ctx0, conv_states_all,
                ...
                (s_slot * mem_size + kv_head) * row_size + off_b);
    ...
    if (cparams.n_rs_seq == 0) {
        ...
        // speculative rollback re-reads one of the last n_rs_seq windows
